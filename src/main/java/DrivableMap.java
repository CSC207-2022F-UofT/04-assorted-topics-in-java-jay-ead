...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed) {
        ...
        Drivable[] valueArray = values.toArray(new Drivable[0]);
        for(int i = 0; i < valueArray.length; i++) {
            if(valueArray[i].getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable(){
        ...
        for(int i = 0; i < valueArray.length; i++) {
            if(valueArray[i] instanceof Tradable){
                tradables.add((Tradable) valueArray[i]);
            }
        }
        return tradables;
    }
